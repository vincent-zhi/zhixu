...
describe("QuotaManager", () => {
  it("allows consumption within free plan limits", () => {
    const manager = new QuotaManager();
    const result = manager.checkQuota("user1", "file_parse", 1);
  ...
  it("denies consumption exceeding quota", () => {
    const manager = new QuotaManager();
    for (let i = 0; i < 10; i++) {
      manager.consumeQuota("user1", "file_parse", 1);
    }
    const result = manager.checkQuota("user1", "file_parse", 1);
  ...
  it("tracks used amount after consumption", () => {
    const manager = new QuotaManager();
    manager.consumeQuota("user1", "model_call", 25);
    const result = manager.checkQuota("user1", "model_call", 1);
    expect(result.usedAmount).toBe(25);
    expect(result.remaining).toBe(25);
  });

  it("returns degradation options when quota exceeded", () => {
    const manager = new QuotaManager();
    for (let i = 0; i < 10; i++) {
      manager.consumeQuota("user1", "file_parse", 1);
    }
  ...
  it("returns empty degradation options when quota is sufficient", () => {
    const manager = new QuotaManager();
    const result = manager.checkQuota("user1", "file_parse", 1);
  ...
  it("isolates quotas per user", () => {
    const manager = new QuotaManager();
    manager.consumeQuota("user1", "file_parse", 10);
    const result = manager.checkQuota("user2", "file_parse", 1);
    expect(result.allowed).toBe(true);
    expect(result.remaining).toBe(10);
  });

  it("applies student_pro plan limits", () => {
    const manager = new QuotaManager();
  ...
  it("applies research_pro plan limits", () => {
    const manager = new QuotaManager();
  ...
  it("defaults unspecified quota types to zero limit", () => {
    const manager = new QuotaManager();
    const result = manager.checkQuota("user1", "storage", 1);
